(ns powertoolmfg.operation-test
  "Smoke tests for the compiled PowerToolOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `powertoolmfg.governor-contract-test`; the Store contract in
  `powertoolmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "PowerToolOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "assy-001" :severity :moderate :description "motor-overheat anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
