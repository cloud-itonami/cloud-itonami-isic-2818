(ns powertoolmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control motor-assembly/housing-molding/
  test-bench equipment directly... does NOT authorize or execute
  ...
  single invariant under test:
    ...
    PowerToolAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Power-Driven Hand Tool
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :drill}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :drill (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest invalid-hipot-test-kv-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:hipot-test-kv 999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-hipot-test-kv} (-> (store/ledger db) last :basis)))
    (is (not= 999999.0 (:hipot-test-kv (store/batch db "batch-001"))) "fabricated hipot-test-kv never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "assy-001" :severity :moderate
                                            :description "motor overheat anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "assy-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
