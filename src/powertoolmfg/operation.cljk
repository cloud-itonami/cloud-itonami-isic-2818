(ns powertoolmfg.operation
  "PowerToolOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (PowerToolAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Power-Driven
  Hand Tool Plant Operations Governor (:govern) and the rollout phase
  ...
  (:require [langgraph.graph :as g]
            ...
            [powertoolmfg.advisor :as advisor]
            [powertoolmfg.governor :as governor]
            [powertoolmfg.phase :as phase]
...
(defn build
  "Compiles a PowerToolOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `powertoolmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PowerToolAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Power-Driven Hand Tool Plant Operations Governor -- independent censor.
      (g/add-node :govern
