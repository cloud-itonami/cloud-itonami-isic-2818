(ns powertoolmfg.sim
  ...
  production quantity, a proposal that tries to ACTUATE motor-
  assembly/housing-molding/test-bench equipment directly (permanently
  blocked, no override), a proposal that tries to self-issue an
  electrical-safety certification mark (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated product type, a
  production-batch patch with an implausible hipot-test-kv reading,
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-type :drill :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on assy-001 (verified, registered, motor-assembly line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on assy-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on molder-002 (UNVERIFIED/unregistered housing-molding press -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on assy-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible hipot-test-kv -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:hipot-test-kv 999999.0}}
                       coordinator))
