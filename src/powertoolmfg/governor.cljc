(ns powertoolmfg.governor
  "Power-Driven Hand Tool Plant Operations Governor -- the independent
  compliance layer that earns the PowerToolAdvisor the right to
  ...
  draft-schedule) motor-assembly/housing-molding/test-bench equipment,
  whether a proposal secretly tries to self-issue an electrical-safety
  ...
  a coordination proposal and becomes direct motor-assembly/housing-
  molding/test-bench-equipment control, so this MUST be a separate
  system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:power-driven-hand-tool-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:motor-line/
                                       actuate` or `:test-bench/run`)
                                       is the 'direct motor-assembly/
                                       housing-molding/test-bench-
    ...
    4. Equipment-actuate blocked   -- for `:schedule-maintenance`, does
                                       ...
                                       motor-assembly/housing-molding/
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`powertoolmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let a
                                       fabricated product type through.
   11. Invalid hipot-test-kv       -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:hipot-test-kv` that is not a
                                       physically plausible double-
                                       insulation withstand-test-
                                       voltage reading
                                       (`powertoolmfg.registry/
                                       hipot-test-kv-valid?`), the
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [powertoolmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct motor-assembly/
  housing-molding/test-bench-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct motor-assembly/housing-molding/test-bench-
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はモータ組立・筐体成形・試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate motor-assembly/housing-molding/test-bench
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :equipment-actuate-blocked
      :detail "モータ組立・筐体成形・試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  compliance) -- an authority exclusively reserved to the accredited
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "電気安全認証(UL/CE等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
...
(defn- invalid-hipot-test-kv-violations
  "For `:log-production-batch`, if the patch declares a
  `:hipot-test-kv` that is not a physically plausible double-
  insulation withstand-test-voltage reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [kv (:hipot-test-kv (:value proposal))]
      (when (and (some? kv) (not (registry/hipot-test-kv-valid? kv)))
        [{:rule :invalid-hipot-test-kv
...
(defn check
  "Censors a PowerToolAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-hipot-test-kv-violations request proposal)
