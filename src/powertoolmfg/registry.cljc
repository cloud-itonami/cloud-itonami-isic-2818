(ns powertoolmfg.registry
  "Pure-function domain logic for the power-driven hand tool (drills,
  saws, sanders and similar hand-held power tools) plant-operations
  ...
  pre-existing `kotoba-lang/powertoolmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `powertoolmfg.governor` -- the same 'ground truth, not self-report'
  ...
  coordinated shipment), not the act of actuating motor-assembly/
  housing-molding/test-bench equipment or dispatching a real freight
  carrier, and never the act of issuing an electrical-safety
  certification mark (this actor NEVER does any of those -- see README
  `What this actor does NOT do`).

  SCOPE: ISIC 2818 covers manufacture of power-driven hand tools --
  motor assembly, housing molding, and testing (including hipot
  double-insulation withstand testing) lines producing finished
  drills, saws, sanders and similar hand-held power tools. This actor
  ...
  flagging, shipment coordination) -- it never touches the motor-
  assembly/housing-molding/test-bench equipment directly, and it never
  stands in for the certification body that issues electrical-safety
...
(def valid-product-types
  ...
  through."
  #{:drill :circular-saw :jigsaw :sander :angle-grinder})

(def hipot-test-kv-min
  "Physical floor for a batch's own hipot (double-insulation withstand)
  test voltage reading, in kV. A test voltage is never negative."
  0.0)

(def hipot-test-kv-max
  "Physical ceiling for a batch's own hipot (double-insulation
  withstand) test voltage reading, in kV. Informed by IEC 60745's
  routine double-insulation withstand-test practice for hand-held
  power tools, which specifies routine test voltages in the low
  single-digit kV range -- this ceiling generously bounds well above
  any real routine test on any standardized class of power-driven hand
  tool; a reading above it is implausible sensor/QC data, not a real
  test."
  15.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own motor-assembly/housing-molding/test
  defect-rate reading (zero defects is the best possible outcome,
  never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own motor-assembly/housing-molding/
  test defect-rate reading -- a batch cannot reject more than 100% of
  its own output. A reading above this is implausible sensor/QC data,
  not a real batch."
  100.0)
...
(defn batch-verified?
  ...
  (i.e. its product-type/hipot-test-kv/quantity/defect-rate claims
...
(defn hipot-test-kv-valid?
  "Is `kv` a physically plausible hipot (double-insulation withstand)
  test voltage reading, in kV? Rejects nil, non-numbers, negative
  values, and values beyond `hipot-test-kv-max` -- a fabricated or
  sensor-error reading, never let through as a real test-result fact."
  [kv]
  (and (number? kv)
       (>= (double kv) hipot-test-kv-min)
       (<= (double kv) hipot-test-kv-max)))

(defn defect-rate-valid?
  "Is `percent` a physically plausible batch motor-assembly/housing-
  molding/test defect-rate reading? Rejects nil, non-numbers, negative
...
(defn register-maintenance
  ...
  motor-assembly/housing-molding/test-bench-equipment maintenance
  window against a verified, registered piece of equipment. Pure
  function -- does not actuate the motor-assembly/housing-molding/
  test-bench equipment or execute any maintenance; it builds the
  RECORD a plant coordinator would keep. `powertoolmfg.governor`
