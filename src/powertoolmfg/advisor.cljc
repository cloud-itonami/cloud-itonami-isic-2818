(ns powertoolmfg.advisor
  "PowerToolAdvisor -- the *contained intelligence node* for the
  power-driven hand tool (drills, saws, sanders and similar hand-held
  power tools) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/hipot-test-kv/
  quantity/defect-rate), drafts a motor-assembly/housing-molding/
  ...
  power-tool shipment coordination proposal against a production
  batch. CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real motor-assembly/housing-molding/
  test-bench-equipment actuation, freight dispatch, or electrical-
  safety certification -- see README `What this actor does NOT do`.
  Every output is censored downstream by `powertoolmfg.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `powertoolmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct motor-
                                 ; assembly/housing-molding/test-bench-
  ...
  `powertoolmfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [powertoolmfg.registry :as registry]
...
(defn- log-production-batch
  ...
  hipot-test-kv, quantity, defect-rate or verification status. High
  confidence, low stakes -- administrative logging, not an
...
(defn- schedule-maintenance
  "Draft a motor-assembly/housing-molding/test-bench-equipment
  ...
  registered?) in its rationale, but `powertoolmfg.governor` NEVER
...
(defn- flag-safety-concern
  "Draft an electrical-safety/motor-overheat/UL-CE-compliance concern.
  ...
  technicality). See `powertoolmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `powertoolmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound power-tool shipment coordination proposal against
  a production batch. The advisor passes through the caller's own
  claimed unit quantity -- it does NOT invent one, and
  `powertoolmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは電動工具(ドリル・丸鋸・サンダー等の可搬式電動手工具)製造工場"
       ...
       "モータ組立・筐体成形・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "電気安全認証(UL/CE等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `powertoolmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :powertoolmfg-advisor-proposal
   :op         (:op request)
