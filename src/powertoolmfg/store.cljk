(ns powertoolmfg.store
  "SSoT for the power-driven hand tool (drills, saws, sanders and
  similar hand-held power tools) plant-operations coordination actor,
  ...
  self-contained (no external power-tool-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/hipot-test-kv/quantity/
    ...
    - `equipment`         -- a motor-assembly/housing-molding/test-
                             bench unit's own record. `:verified?`/
    ...
    - `maintenance`       -- a scheduled motor-assembly/housing-
                             molding/test-bench maintenance-window
                             DRAFT against a piece of equipment
                             (`powertoolmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound power-tool shipment
                             DRAFT (`powertoolmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (powertoolmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (powertoolmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :drill
                :model "DRL-18V" :hipot-test-kv 2.5
                :quantity-units 1000.0 :defect-rate-percent 1.2
                :verified? true :registered? true
                :shipped-units 200.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :circular-saw
                :model "CSW-185MM" :hipot-test-kv 3.0
                :quantity-units 300.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-units 280.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :sander
                :model "SND-125MM" :hipot-test-kv 1.5
...
(defn- sample-equipment []
  {"assy-001" {:id "assy-001" :kind :motor-assembly-line
               :verified? true :registered? true
               :last-maintenance-date "2026-05-01"}
   "molder-002" {:id "molder-002" :kind :housing-molding-press
                 :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `powertoolmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `powertoolmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  motor-assembly-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered housing-molding-press unit (blocks any
